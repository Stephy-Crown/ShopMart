...
const BusinessSignUpForm = ({ toggleMode }) => {
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [fullName, setFullName] = useState("");
  const [profileImage, setProfileImage] = useState(null);
  ...
  return (
    <div className="w-full max-w-[85vw] lg:max-w-md p-8 space-y-4 bg-white rounded-xl shadow-lg">
      <h2 className="text-lg lg:text-2xl font-bold text-center text-[#3D593E] ">
      ...
      <form onSubmit={handleSignUp} className="space-y-4 lg:space-y-6">
        ...
        <button
          type="submit"
          className="w-full flex justify-center px-4 py-2 font-medium text-white hover:bg-gradient-to-r   bg-gradient-to-r from-[#557C56] to-[#3D593E] hover:bg-[##3D593E] rounded focus:outline-none focus:ring-2 "
        >
          {loadingEmail ? (
            <div className="border-2 border-gray-300 border-t-2 border-t-transparent rounded-full w-5 h-5 spinner"></div>
          ) : (
